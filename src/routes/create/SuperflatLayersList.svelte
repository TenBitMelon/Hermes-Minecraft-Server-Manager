...
<div class="flex flex-col items-stretch gap-2 rounded-md bg-gray-800 p-2">
  {#each list as n, index (n.id)}
    ...
    <div on:dragover={(event) => event.preventDefault()} role="listitem" class="flex gap-2 rounded-md px-1" draggable={true} on:dragstart={(event) => dragstart(event, index)} on:drop|preventDefault={(event) => drop(event, index)} on:dragenter={() => (hovering = index)} class:is-active={hovering === index}>
      ...
      <input class="w-full rounded-md bg-gray-700 px-4 py-2 transition-all hover:bg-gray-650 focus:outline-none focus:ring-2 focus:ring-primary" bind:value={n.block} placeholder="Layer Block" />
      <input class="w-20 rounded-md bg-gray-700 py-2 pl-4 pr-2 text-center transition-all hover:bg-gray-650 focus:outline-none focus:ring-2 focus:ring-primary" min="1" max="1000" type="number" bind:value={n.height} />
  ...
  {/each}
  <button on:click={() => (list = [...list, { block: 'minecraft:dirt', height: 1, id: nextId++ }])} type="button" class="rounded-md bg-gray-700 p-1 px-4 transition-all hover:bg-gray-650">Add layer</button>
</div>
